...
| `0.4.1` | 2026-07-07 | `e0b8dcd` | **Homey App Store (privater Test)** | M4 | Store-Politur nach erstem Test: Titel `&`/`and` → `+` (beide Sprachen), README en+de auf Absätze in Einzelzeilen umgestellt (Store hatte harte Umbrüche gerendert), App-Icon als **reines Vektor-SVG** neu aufgebaut (`#7300AF` + weißes U/Trennlinie/UIOLET) — das alte SVG bettete ein PNG via `href` ein, das Homeys Server-Renderer nicht lud → runder Store-Icon-Kreis blieb leer. |

**Nächster Upload: `0.4.2`** (Patch im selben Milestone) **oder** `0.5.0` (Milestone M5).
