...
**Nächster Upload: `0.4.4`** (Patch) **oder** `0.6.0` (Milestone M6). **Vor Store-Publish offen:** Write-Passwort rotieren + `docs/superpowers/security/credential-rotation.md` mit Datum anlegen (release-gate (c)); **vor Live** zusätzlich `homeyCommunityTopicId` ins Manifest (Forum-Thread-ID aus den Launch-Posts).
