...
**Stand:** 2026-08-30 (Cloud-Triage-Lauf)
...
- 2026-08-30 · Lauf [Cloud] · npm ci sauber (5 packages) · npm test 441 Tests / pass 439 / fail 0 / skipped 2 / todo 0 (6,4 s) · Versions-Sync app.json == .homeycompose/app.json OK (0.8.0) · letzter CI-Lauf 33253830348 (main, completed/success, 2026-08-29T12:58:36Z), head_sha 82462c6 == aktueller HEAD nach Sync · `gh`-CLI weiterhin nicht installiert, GitHub-MCP-Tools als Ersatz genutzt (erwartet, kein Handlungsbedarf).
- 2026-08-30 · Befund [Cloud] · `npx homey app validate --level publish` weiterhin durch `package-guard`-Hook blockiert (fail closed, npm-Registry aus der Cloud-VM nicht erreichbar) — unverändert, keine neue Information; Entscheidung zur Entfernung aus der Routine läuft unter „Offen" (2026-08-25-Eintrag) weiter, wartet auf manuellen Edit des Routine-Prompts.
- 2026-08-30 · Befund [Cloud] · **Detached-HEAD zweite Nacht in Folge** (nach Wiederauftreten am 2026-08-29): Checkout stand zu Laufbeginn erneut auf `HEAD detached from refs/heads/main` (bei 82462c6), lokale `main`-Ref war zusätzlich 10 Commits hinter dem detached HEAD (774d440) — nach `git fetch`/`git checkout main && git merge --ff-only origin/main` deckungsgleich mit `origin/main`. Kein Content-Schaden, aber zwei Nächte in Folge verschärfen die offene Design-Frage von „hypothetisch" zu „wiederkehrend". Siehe aktualisierten Eintrag „Offen" (2026-07-14).

## Erledigt (2026-08-30)

- Nightly-Lauf 2026-08-29 ([Cloud], Kernergebnisse grün inkl. bekannten Befunden: validate blockiert, `gh` fehlt; Detached-HEAD-Symptom weiterhin unter „Offen" 2026-07-14 nachgehalten) → aus „Neu" entfernt, kein neuer Handlungsbedarf über bereits Dokumentiertes hinaus.
