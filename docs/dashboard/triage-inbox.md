...
**Stand:** 2026-09-03 (Live-Test-Fund 0.9.0 behoben in 0.9.1, nach Cloud-Triage-Lauf 2026-09-02)

## Offen

- 2026-09-03 · Live-Test/Defekt (ERLEDIGT 2026-09-03, siehe unten) · **Repair-Dialog öffnet nicht:** in der Homey-App wirft Gerät → Wartung → Reparieren sofort `Error: unknown_error_getting_file` (Nutzerbeobachtung auf 0.9.0, Torstens Homey Pro). Der Repair-Flow aus PR #19 (N5/F-A, `drivers/pool/pair/repair.html` + `repair`-View in `driver.compose.json`) ist damit live unbenutzbar; die Annahme „Repair-Dialog inkl. Homey.done()" bleibt unverifiziert. Verdacht: die Pairing-Runtime findet die View-Datei nicht — passt zu R2-3 (View-Asset-Verhalten unverifiziert) und evtl. zum ungeklärten ersten `homey app install`-Versuch derselben Session („× Missing File", zweiter Versuch OK). Beleg: docs/dashboard/versions.md (Zeile 0.9.0). **Stand 2026-09-03, reproduziert per `homey app run`** (Docker-Dev-Modus, 0.9.0 remote): Repair-Klick wirft erneut `unknown_error_getting_file`, und der App-Log bleibt dabei **komplett stumm** — nach „Pool device initialized" / „NOTIFY listener bound on port 22222" (14:30:48Z) kommt keine weitere Zeile, `onRepair` (driver.js:147) wird nie erreicht. Der Fehler entsteht also in der Homey-Runtime beim Laden der View-Datei, nicht im App-Code. **Ursachen-Hypothese mit Beleg aus der CLI-Quelle** (`homey/lib/HomeyCompose.js:303–332`): Compose materialisiert Repair-Views nach `drivers/<id>/repair/<viewId>.html` (Pair-Views nach `drivers/<id>/pair/`), die Runtime sucht Repair-Views folglich im `repair/`-Ordner — unsere Custom-View liegt aber als `drivers/pool/pair/repair.html`, `drivers/pool/repair/` existiert nicht. Fix-Kandidat: Datei nach `drivers/pool/repair/repair.html` verschieben (plus Test, der die Ordner-Konvention pinnt); R2-3 (Asset-Serving aus dem View-Ordner) wird dabei gleich mitgeprüft. Runtime-Lookup selbst nicht einsehbar — Hypothese, erst der verschobene Build beweist sie.
...
- 2026-09-02 · Lauf [Cloud] · npm ci sauber (5 packages) · npm test 461 Tests / pass 459 / fail 0 / skipped 2 / todo 0 (5,996 s) · Versions-Sync app.json == .homeycompose/app.json OK (0.8.0) · letzter CI-Lauf 33484004286 (#163, main, schedule, completed/success, 2026-09-01T07:49:34Z), head_sha 6415c42 == aktueller HEAD nach Sync · `gh`-CLI weiterhin nicht installiert, GitHub-MCP-Tools als Ersatz genutzt (erwartet, kein Handlungsbedarf).
- 2026-09-02 · Befund [Cloud] · `npx homey app validate --level publish` weiterhin durch `package-guard`-Hook blockiert (fail closed, npm-Registry aus der Cloud-VM nicht erreichbar) — unverändert, keine neue Information; Entscheidung zur Entfernung aus der Routine läuft unter „Offen" (2026-08-25-Eintrag) weiter, wartet auf manuellen Edit des Routine-Prompts.
- 2026-09-02 · Befund [Cloud] · **Detached-HEAD wieder aufgetreten**, direkt nach dem einzigen sauberen Lauf (09-01) — Checkout stand zu Laufbeginn erneut auf `HEAD detached from refs/heads/main` (bei 6415c42, dem bereits gepushten Vortags-Commit); die lokale `main`-Ref war diesmal die veraltete Seite (b66c425), nicht der detached HEAD. Nach `git fetch origin main && git checkout -B main origin/main` deckungsgleich, kein Content-Schaden. 4 von 6 protokollierten Läufen zeigen das Symptom — Details im aktualisierten Eintrag „Offen" (2026-07-14).
...
- Nightly-Lauf 2026-09-01 ([Cloud], Kernergebnisse grün: npm ci sauber, npm test 461/459/0/2/0 (6,16 s), Versions-Sync OK 0.8.0, CI-Lauf 33394109943 grün; bekannte Befunde: validate blockiert, `gh` fehlt; Detached-HEAD trat ausnahmsweise NICHT auf — einziger sauberer Lauf der bisherigen Serie, siehe „Offen" 2026-07-14) → aus „Neu" entfernt, kein neuer Handlungsbedarf über bereits Dokumentiertes hinaus.
...
- Nightly-Lauf 2026-08-31 ([Cloud], Kernergebnisse grün: npm ci sauber, npm test 448/446/0/2/0 (6,85 s), Versions-Sync OK 0.8.0, CI-Lauf 33337614435 grün; bekannte Befunde: validate blockiert, `gh` fehlt; Detached-HEAD dritte Nacht in Folge — Details in „Offen" 2026-07-14 nachgehalten; 7/7-Zählung „Parallelbetrieb Cloud vs. lokal" erreicht, Entscheidung an Checkpoint-Session verwiesen) → aus „Neu" entfernt, kein neuer Handlungsbedarf über bereits Dokumentiertes hinaus.
