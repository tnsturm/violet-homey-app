...
**Stand:** 2026-08-29 (Cloud-Triage-Lauf)
...
- 2026-08-29 · Lauf [Cloud] · npm ci sauber (5 packages) · npm test 408 Tests / pass 406 / fail 0 / skipped 2 / todo 0 (7,6 s) · Versions-Sync app.json == .homeycompose/app.json OK (0.8.0) · letzter CI-Lauf 33179757986 (main, Run #149, schedule) completed/success (2026-08-28T14:21:40Z), head_sha 556faae == aktueller HEAD · `gh`-CLI weiterhin nicht installiert, GitHub-MCP-Tools als Ersatz genutzt (erwartet, kein Handlungsbedarf).
- 2026-08-29 · Befund [Cloud] · `npx homey app validate --level publish` weiterhin durch `package-guard`-Hook blockiert (fail closed, npm-Registry aus der Cloud-VM nicht erreichbar) — unverändert, keine neue Information; Entscheidung zur Entfernung aus der Routine läuft unter „Offen" (2026-08-25-Eintrag) weiter, wartet auf manuellen Edit des Routine-Prompts.
- 2026-08-29 · Befund [Cloud] · **Detached-HEAD wieder aufgetreten** (zuletzt beobachtet, am 2026-07-21 als „nicht reproduzierbar" geschlossen): Checkout stand zu Laufbeginn auf `HEAD detached from refs/heads/main`, nicht auf Branch `main` wie der Routinen-Prompt annimmt (`git branch --show-current` leer). Inhaltlich kein Schaden — HEAD war bit-identisch mit `origin/main` (556faae) nach `git fetch`, Testzahlen also korrekt; die lokale `origin/main`-Tracking-Ref war vor dem Fetch zudem 8 Commits veraltet (774d440 statt 556faae), was den Verdacht divergenter unqueshter Commits kurzzeitig aufwarf — per GitHub-API bestätigt gegenstandslos, reines Fetch-Timing. Siehe aktualisierten Eintrag „Offen" (2026-07-14).
