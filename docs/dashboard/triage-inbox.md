...
**Stand:** 2026-08-24 (Checkpoint-Bearbeitung, kein Triage-Lauf)
...
- 2026-08-24 · Routine/Beobachtung (NEU, offen) · **Parallelbetrieb Cloud vs. lokal beobachten.** Der Cloud-Teil ist angelegt (siehe Kopf) und lief erstmals am 2026-08-24. Der Plan sah „erst eine Woche parallel zum lokalen Task, dann den lokalen abschalten" vor — **das ist gegenstandslos, weil der lokale Task bereits deaktiviert ist** (siehe den Lücken-Eintrag darunter). Stattdessen gilt: **7 Cloud-Läufe in Folge mit `chore(triage): nightly cloud`-Commit auf `main` abwarten** (Prüfung: `git log --grep="chore(triage): nightly cloud" --oneline` bzw. die Lauf-Historie unter https://claude.ai/code/routines/trig_01B8QrhLtN8zvdfWuF5iAmNP — die hat die lokale Scheduler-API nie geliefert und war der Grund für den Umzug). Erst danach entscheiden, ob der lokale Task für den Live-Smoke-Teil wieder aktiviert wird oder ganz entfällt. Offen und ungeprüft: ob `npx homey app validate --level publish` in der Cloud-VM ohne Homey-Login durchläuft — die Homey-CLI ist keine devDependency, der erste Lauf notiert das Ergebnis selbst.
...
- 2026-08-17 · Lauf · **ALLES GRÜN** (auf `main`): npm test 366 Tests / pass 366 / fail 0 / todo 0 (14,0 s) · `homey app validate --level publish` exit 0 · Versions-Sync app.json == .homeycompose/app.json (0.8.0) · CI-Lauf 31990453384 (main, schedule) completed/success (24 s) · Worktree nach `validate` sauber (elfte Bestätigung des `.gitattributes`-Fixes aus `57e0b4e`).
- 2026-08-17 · Routine/Lücke · Zwölf Nächte ohne Lauf (08-05…08-16) — Details und Handlungsvorschlag siehe „Offen".
