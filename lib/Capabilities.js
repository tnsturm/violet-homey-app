'use strict';

// Capability mapping & per-poll update planning (pure) — spec §5, §7, §8, §9
// (docs/superpowers/specs/2026-06-24-violet-homey-app-m0-foundation-design.md).
// Translates normalized readings + detected features into the capability set the
// Pool device should expose and the values to write each poll. Fresh-gating of
// ph/orp/chlorine lives here (§7); device.js just applies the result.

// Feature key → custom capability id. M0 wires only chlorine; M2 extends this (§9).
const FEATURE_CAPABILITY = {
  chlorine: 'measure_chlorine',
};

/**
 * Sub-capability id for a temperature channel (spec §8).
 * @param {number} id 1-wire channel number.
 * @returns {string} e.g. `measure_temperature.ow3`.
 */
function channelSubCapId(id) {
  return `measure_temperature.ow${id}`;
}

// A onewire label that denotes the pool-water sensor (M5.7 0.5.1 smart-auto,
// spec addendum). User-entered free text, so match loosely on the common
// German/English pool words; ambiguity (≥2 matches) falls back to the count rule.
const POOL_NAME_RE = /schwimm|pool|becken|wasser/i;

/**
 * Pick the value for the primary water-temperature capability (spec §8; M5.7
 * 0.5.1 smart-auto). A specific channel id always wins. With "auto"/unset:
 * when config names are available, auto-selects the single OK channel whose
 * NAMES_onewire label looks like the pool; otherwise (no/ambiguous name match)
 * falls back to the historical rule — the sole OK channel, else null.
 * @param {Array<{id: number, value: number}>} tempChannels OK channels.
 * @param {string|number|null} selectedChannel Setting value or "auto".
 * @param {?Object<string, string>} [onewireNames] Channel id → user label, or null.
 * @returns {?number} Chosen temperature, or null.
 */
function choosePrimaryTemperature(tempChannels, selectedChannel, onewireNames = null) {
  if (selectedChannel === 'auto' || selectedChannel === null || selectedChannel === undefined) {
    if (onewireNames) {
      const named = tempChannels.filter((c) => POOL_NAME_RE.test(onewireNames[String(c.id)] || ''));
      if (named.length === 1) return named[0].value;
    }
    return tempChannels.length === 1 ? tempChannels[0].value : null;
  }
  const match = tempChannels.find((c) => c.id === Number(selectedChannel));
  return match ? match.value : null;
}

/**
 * Resolve which feature capabilities should be present (spec §9).
 * Per feature: "force" always shows, "auto" shows iff detected, else hidden.
 * @param {{features: import('./FeatureDetector').Features, overrides?: Object<string, string>}} args Detected features + per-group mode.
 * @returns {string[]} Capability ids that should be present.
 */
function desiredFeatureCapabilities({ features, overrides }) {
  const caps = [];
  for (const [feature, capId] of Object.entries(FEATURE_CAPABILITY)) {
    const mode = (overrides && overrides[feature]) || 'auto';
    const present = mode === 'force' || (mode === 'auto' && !!(features && features[feature]));
    if (present) caps.push(capId);
  }
  return caps;
}

/**
 * Build the per-poll capability→value map (spec M0 §5, §7; clear-on-stale:
 * 2026-06-27-m0-clear-stale-measurements-design.md §3).
 * pump_running, measurements_fresh and temperature update every poll. ph/orp/
 * chlorine carry their fresh value while `fresh`; while stale they are set to
 * `null` so the GUI shows "–" and Insights records a gap instead of holding the
 * last fresh value as a flat line (§3).
 * @param {{parsed: {pumpOn: boolean, tempChannels: Array<{id: number, value: number}>, ph: ?number, orp: ?number, chlorine: ?number}, fresh: boolean, primaryChannel: ?number, lsi?: number|null, alarm?: boolean|null}} args
 *   `parsed` is the subset of ParsedReadings this planner reads (checkJs strict, M5 gate c).
 *   `lsi`/`alarm` may be omitted (checkJs, M4.5): the `??` fallbacks below make
 *   omission equivalent to null/false — callers without the LSI feature skip them.
 * @returns {Object<string, *>} Capability id → value. Apply rule (in device.js):
 *   `undefined` = leave as-is, `null` = clear to empty, else set.
 */
function buildCapabilityUpdates({ parsed, fresh, primaryChannel, lsi, alarm }) {
  const updates = /** @type {Object<string, *>} */ ({});
  // Direction-dependent flag position (review F5 + diff review F-B): the flag
  // must never certify more than currently holds. Stale edge → revoke FIRST
  // (before the null-clears); fresh edge → publish LAST (after the values).
  if (!fresh) updates.measurements_fresh = false;
  Object.assign(updates, {
    pump_running: parsed.pumpOn,
    measure_temperature: primaryChannel,
    // LSI (M1 §6,§9): number when enabled+fresh+inputs complete, else null
    // (cleared to "–"/Insights gap). Capability may be absent (lsi_enabled off)
    // — device.js skips absent caps. `?? null` keeps a valid 0.
    measure_lsi: lsi ?? null,
    // Water-balance alarm (M1 §7.3): boolean, true when the LSI is outside the
    // balanced band. Absent when lsi_enabled off (device.js skips absent caps).
    alarm_water_balance: alarm ?? false,
  });
  for (const ch of parsed.tempChannels) {
    updates[channelSubCapId(ch.id)] = ch.value;
  }
  if (fresh) {
    updates.measure_ph = parsed.ph;
    updates.measure_orp = parsed.orp;
    if (parsed.chlorine !== null) updates.measure_chlorine = parsed.chlorine;
  } else {
    // Stale: clear probes so the GUI shows "–" and Insights gaps instead of
    // carrying the last fresh value forward (clear-stale §3; M0 §7).
    updates.measure_ph = null;
    updates.measure_orp = null;
    updates.measure_chlorine = null;
  }
  // Fresh edge: publish the flag LAST (review F5) — a Flow gating on
  // measurements_fresh must never fire before the values it certifies exist.
  // (Stale already placed the key first, above.) device.js additionally
  // enforces the same direction rule at apply time across the merged M2 batch.
  if (fresh) updates.measurements_fresh = true;
  return updates;
}

/**
 * Debounce capability removal (review 2026-08-28 F2; TIME-based since the
 * 2026-08-29 diff review F-C: counting _tick() invocations let a multi-key
 * settings save — 2–3 ticks within one second — burn the whole grace on a
 * single transient fault). A capability is torn down only when it has been
 * absent from the desired set for at least `graceMs` of continuous wall time;
 * at graceMs = 2× the poll interval the 3rd regular poll is the first
 * observation at/after the grace ("3 consecutive polls without evidence",
 * mirroring the 3-failure availability rule, M0 §10). Mutates `absentSince`
 * (the caller owns the per-device map); a `wanted` observation resets.
 * @param {Object<string, number>} absentSince capId → epoch ms of the first absent observation.
 * @param {string} cap Capability id.
 * @param {boolean} wanted Present in this poll's desired set.
 * @param {number} nowMs Current time (epoch ms) — injected for testability.
 * @param {number} graceMs Continuous absence required before removal.
 * @returns {boolean} True when the cap should be removed now.
 */
function shouldRemoveAfterAbsence(absentSince, cap, wanted, nowMs, graceMs) {
  if (wanted) { delete absentSince[cap]; return false; }
  if (!(cap in absentSince)) { absentSince[cap] = nowMs; return false; }
  return nowMs - absentSince[cap] >= graceMs;
}

module.exports = {
  FEATURE_CAPABILITY,
  channelSubCapId,
  choosePrimaryTemperature,
  desiredFeatureCapabilities,
  buildCapabilityUpdates,
  shouldRemoveAfterAbsence,
};
