 ...
 * Leere Pipe-Segmente ("0|", "0||X") werden verworfen.
 * @param {*} v Raw field value.
...
// Shared numeric coercion (review Q6): the byte-identical private mirror is
// gone — one implementation, one place to fix.
const { num } = require('./VioletClient');
...
/**
 * Split a capability id into base and channel at the first dot (review Q10 —
 * previously reimplemented at six call sites with inconsistent channel
 * defaults). Single place to change if the sub-capability convention moves.
 * @param {string} capId e.g. `alarm_dosing_blocked.cl` or `cover_state`.
 * @returns {{base: string, ch: ?string}} `ch` is null for undotted ids.
 */
function splitCapId(capId) {
  const dot = capId.indexOf('.');
  return dot > 0 ? { base: capId.slice(0, dot), ch: capId.slice(dot + 1) } : { base: capId, ch: null };
}
...
function diagAnnotatable(capId) {
  ...
  return (ch !== null && Object.prototype.hasOwnProperty.call(DIAG_DOSING, base))
