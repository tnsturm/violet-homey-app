...
const { parseDurationToHours, DOSING_PREFIX } = require('./FeatureGroups');
 ...
 *  adcChannels:Array<{id:number, use:boolean, units:string, name:string, decimals:?number}>,
 *  impulsChannels:Array<{id:number, use:boolean, units:string, name:string}>}. Index signature because
...
function detectFeatures(raw, configFacts = null) {
  ...
  // Single source of truth for the channel→prefix map (review Q2): the same
  // registry buildM2Updates reads — a new dosing channel lands in one place.
  const dosingChannels = Object.keys(DOSING_PREFIX).filter((ch) => raw[`${DOSING_PREFIX[ch]}_USE`] === '1');
