...
// Version der persistierten ConfigFacts-Form (M5.8 spec §4): bei jeder
// Shape-Änderung (neue Felder je Kanal) erhöhen — das Device verwirft dann
// gespeicherte Facts älterer Form und liest getConfig einmalig neu.
const FACTS_SCHEMA_VERSION = 2; // 1 = M5.7 (ohne impuls name/adc decimals), 2 = M5.8
...
function parseConfigFacts(rawConfig) {
  ...
  for (let id = 1; id <= 6; id += 1) {
    ...
    const decRaw = raw[`ANALOG_adc${id}_decimal`];
    const dec = decRaw === null || decRaw === undefined || decRaw === '' ? NaN : Number(decRaw);
    adcChannels.push({
      id,
      use,
...
module.exports = { CONFIG_QUERY, FACTS_SCHEMA_VERSION, buildConfigUrl, parseConfigFacts, fetchConfigFacts, createConfigLogThrottle, factsEmpty };
