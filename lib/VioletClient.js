...
/**
 * @typedef {Object<string, string|number|Array<string>|undefined>} RawReadings
 * Parsed getReadings?ALL payload — Violet field name → raw value. The field set is
 * hardware-dependent, so it is an index signature and every access must
 * null/undefined-check (checkJs strict-ratchet, M5 gate c).
 */

/**
 * @typedef {object} ParsedReadings
 * @property {?number} ph
 * @property {?number} orp
 * @property {?number} chlorine
 * @property {boolean} pumpOn
 * @property {Array<{id: number, value: number, state: string}>} tempChannels
 * @property {?number} timeUnix
 * @property {?number} pumpLastOn
 * @property {RawReadings} raw
 */
...
/**
 * Normalize a user-entered host (pair/repair views, review 2026-08-28 N10):
 * users paste URLs — strip the scheme and trailing slashes so buildReadingsUrl
 * never yields `http://http://…` and the dialog never shows raw fetch errors
 * for a self-inflicted double scheme.
 ...
 * Exported (review Q6): FeatureGroups consumes the same coercion for M2 fields
 * instead of keeping a byte-identical private copy.
 ...
 * @param {RawReadings} raw Parsed JSON from getReadings?ALL.
 * @returns {ParsedReadings} Normalized readings.
 */
 ...
 * @returns {Promise<RawReadings>} Parsed JSON payload.
 */
async function fetchReadings(host, { timeoutMs = 10000 } = {}) {
  ...
  try {
    ...
    return /** @type {RawReadings} */ (await res.json());
  } finally {
    clearTimeout(timer);
  }
}

module.exports = { buildReadingsUrl, normalizeHost, num, parseReadings, fetchReadings };
