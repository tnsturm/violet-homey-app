...
// Shared surrogate guard (review R2-1) — pure lib, no Homey dependency.
const { stripTrailingHighSurrogate } = require('./WaterBalanceText');
...
function parseAlarm(method, url, body) {
  ...
  // The code-unit cut may split a surrogate pair (review N8) — the shared
  // guard (R2-1) drops the dangling half before it re-encodes as U+FFFD.
  const subject = stripTrailingHighSurrogate(rawSubject
    .replace(/[\u0000-\u001f\u007f-\u009f\u2028\u2029]+/g, ' ')
    .trim()
