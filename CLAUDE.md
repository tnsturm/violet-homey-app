...
3. Based on the result, ask (don't decide silently): **trivial change (no Critical Issues)** — whether to push directly to `origin/main` and pull the local `main` up to date, skipping a PR; **otherwise** — whether to push the branch and open a Pull Request.
