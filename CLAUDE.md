...
**Framework-Artefakte** (seit M4.9): Jede GENERISCHE Änderung an CLAUDE.md-Protokollregeln, dem Checkpoint-Ablauf, den Dashboard-/Template-Formaten oder einem projektlokalen Hook-Muster wird **in derselben Session** in das Framework-Repo `skill-agentic-loop-framework` (`plugin/skills/agentic-loop-framework/templates/` bzw. `homey/`) gespiegelt + dessen CHANGELOG ergänzt. Die generischen Gate-Hooks (test-gate, typecheck-gate, commit-msg-guard, package-guard, control-bytes-guard, dashboard-guard, run-matching-test, selftest-guards, handoff-notice) liegen seit M9.4 **nur noch im Plugin** (`plugin/hooks/`, Opt-in über `enabledPlugins` in `.claude/settings.json`) — eine Änderung daran ist eine Plugin-Änderung + Plugin-Release, die dieses Repo per `claude plugin update agentic-loop-framework` bezieht; in `.claude/hooks/` bleiben nur Homey- und projektspezifische Hooks. Projekt-Spezifisches (Violet-Live-Smoke, konkrete Allowlist-Einträge) bleibt hier. Der `milestone-checkpoint` prüft Drift (Schritt 7a) nur noch für das, was in beiden Repos liegt.
